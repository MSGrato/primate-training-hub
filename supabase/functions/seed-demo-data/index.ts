...
Deno.serve(async (req) => {
  ...
  // Authenticate the request - require a valid user with coordinator role
  ...
  const { data: roleData } = await supabaseAuth.from("user_roles").select("role").eq("user_id", userId).eq("role", "coordinator").maybeSingle();
  if (!roleData) {
