  ...
  try {
    ...
    if (intent === "general") {
      const aiSummary = await generateAISummary(
        prompt,
        `The user is a ${callerRole}. No specific training data was queried.`,
      );
      return new Response(
        JSON.stringify({
          intent,
          ...
          suggested_prompts: [
            ...
            "What trainings should I prioritize?",
          ],
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    ...
    if (intent === "training_search") {
      ...
      const scored = (trainings ?? [])
        .map((t) => {
          const haystack = [t.title ?? "", t.description ?? "", t.category ?? "", t.frequency ?? ""]
            .join(" ")
            .toLowerCase();
          const score =
            queryTokens.length === 0 ? 1 : queryTokens.reduce((c, tk) => c + (haystack.includes(tk) ? 1 : 0), 0);
      ...
      const dataCtx = `Found ${matches.length} trainings matching "${query}". Top results: ${matches
        .slice(0, 10)
        .map((m) => `${m.training_title} (due: ${m.due_date ?? "N/A"})`)
        .join(", ")}`;
    ...
    if (intent === "employee_search") {
      ...
      const filtered =
        queryTokens.length === 0
          ? empRows
          : empRows.filter((r) => {
              const haystack = [r.net_id, r.full_name, r.job_title, r.role, r.supervisor, r.tags, r.is_active]
                .join(" ")
                .toLowerCase();
      ...
      const dataCtx = `Found ${resultRows.length} employees${queryTokens.length > 0 ? ` matching "${query}"` : ""}. Employee details: ${resultRows
        .slice(0, 15)
        .map(
          (r) =>
            `${r.full_name} (${r.net_id}), ${r.job_title}, ${r.role}, supervisor: ${r.supervisor}, tags: ${r.tags || "none"}, ${r.is_active}`,
        )
        .join("; ")}`;
      const aiSummary = await generateAISummary(prompt, dataCtx);
      ...
      return new Response(
        JSON.stringify({
          ...
          scope: {
            role: callerRole,
            users: resultRows.length,
            dueSoonDays: dueSoonDays,
            net_id_filter: requestedNetId,
          },
          highlights: {
            total_assignments: resultRows.length,
            compliant: 0,
            overdue: 0,
            due_soon: 0,
            not_started: 0,
          ...
          suggested_prompts: [
            ...
            "Who has the most overdue trainings?",
    ...
    const detailedRows = assignments
      ...
      .map((a) => {
        ...
        const jobTitleName = profile?.job_title_id
          ? (jobTitleNameById.get(profile.job_title_id) ?? "Unassigned")
          : "Unassigned";
    ...
    const completionRate =
      totals.total_assignments === 0 ? 0 : Number(((totals.compliant / totals.total_assignments) * 100).toFixed(1));

    // ── Build rows based on intent ──
    const byJobTitle = new Map<
      string,
      { total: number; compliant: number; overdue: number; due_soon: number; not_started: number }
    >();
    for (const row of detailedRows) {
      const current = byJobTitle.get(row.job_title) ?? {
        total: 0,
        compliant: 0,
        overdue: 0,
        due_soon: 0,
    ...
    const jobTitleBreakdown = Array.from(byJobTitle.entries())
      .map(([job_title, v]) => ({
        job_title,
        ...v,
        completion_rate: v.total === 0 ? 0 : Number(((v.compliant / v.total) * 100).toFixed(1)),
      }))
    ...
    const dataContext = [
      ...
      jobTitleBreakdown.length > 0
        ? `Job title breakdown: ${jobTitleBreakdown.map((j) => `${j.job_title}: ${j.completion_rate}% compliant, ${j.overdue} overdue`).join("; ")}`
        : "",
      intent === "overdue"
        ? `Overdue trainings: ${(rows as any[])
            .slice(0, 15)
            .map((r) => `${r.full_name} - ${r.training_title}`)
            .join("; ")}`
        : "",
      intent === "due_soon"
        ? `Due soon trainings: ${(rows as any[])
            .slice(0, 15)
            .map((r) => `${r.full_name} - ${r.training_title} (due ${r.next_due_at})`)
            .join("; ")}`
        : "",
      intent === "recommendations"
        ? `Non-compliant items: ${(rows as any[])
            .slice(0, 20)
            .map((r) => `${r.full_name} - ${r.training_title} (${r.status})`)
            .join("; ")}`
        : "",
